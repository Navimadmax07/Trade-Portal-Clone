...
const dummyResult = {
  doc: {
    ThRemedyStatus: true,
    ThNtlc: "1001",
    ThNTLCDescription: "Wheat and meslin",
    TradeRemediesYear: "2024",
    CollectionYear: "2023",
    TradeRemedyData: "Anti-dumping duty applies.",
  },
};

const dummyNoRemedy = {
  doc: {
    ThRemedyStatus: false,
  },
};
            ...
            <div className="card-body">
              <span style={{ display: "inline-flex" }}>
                ...
                <i
                  ...
                  aria-hidden="true"
                  title="Trade remedies  are trade policy tools that allow governments to take remedial action against imports."
                  style={{ marginLeft: 8, marginTop: 10 }}
                ></i>
              </span>
              {result && result.doc ? (
                result.doc.ThRemedyStatus ? (
                  <>
                    <p>
                      For product {result.doc.ThNtlc} -{" "}
                      {result.doc.ThNTLCDescription}
                    </p>
                    <p>Exported from India to Destination Country</p>
                    <p>Trade year : {result.doc.TradeRemediesYear}</p>
                    <p>Collection year : {result.doc.CollectionYear}</p>
                    <p>Trade Remedy Data : {result.doc.TradeRemedyData}</p>
                  </>
                ) : (
                  <p style={{ fontSize: 18 }}>
                    Selected Country does not apply any trade remedy on the
                    selected product.
                  </p>
                )
              ) : null}
